...
const displayNameSuffix = 'Power Status';
const serialSuffix = 'power-status';

/**
 * This accessory is used to show the power usage of the Energy Socket above or below a threshold.
 *
 * This is just a way around the limitation of the Home App for not doing anything useful with the OutletInUse characteristic.
 *
 * This accessory will show up as a normal Outlet in the Home App, but is actually not a real Outlet.
 * It will turn ON when the power usage is above a threshold, and turn OFF when the power usage is below a threshold.
 *
 * You cannot turn this accessory ON or OFF, it will always be ON or OFF based on the power usage.
 */
export class EnergySocketAccessoryPowerStatus {
  ...
  constructor(
    ...
    informationService?.setCharacteristic(
      ...
      this.properties.serialNumber + '-' + serialSuffix, // Like: "1c23e7280952-power-status", using a suffix to make it unique
    );
    informationService?.setCharacteristic(
    ...
    this.service.setCharacteristic(
      ...
      this.properties.displayName + ' ' + displayNameSuffix,
    );
  ...
  get log() {
    const loggerPrefix = `[Energy Socket: ${
      this.properties.displayName + ' ' + displayNameSuffix
    }] -> `;
  ...
  setOutletInUse(value: boolean, activePower: number | null | undefined) {
    ...
    // Set the On characteristic to true or false, because this accessory is just a way to show the power usage above or below a threshold
    this.service.setCharacteristic(this.platform.Characteristic.On, value);
  ...
  async handleSetOn(_: CharacteristicValue): Promise<void> {
    try {
      ...
      // Important: do not do a API call here, because this accessory is just a way to show the power usage above or below a threshold

      this.log.info(
        `On state not updated, because this accessory is just a way to show the power usage above or below a threshold.`,
      );
    } catch (error) {
  ...
  async handleGetOn(): Promise<CharacteristicValue> {
    try {
      const isOn = !!(
        this.isThresholdCrossedAboveAfterDuration || this.isThresholdCrossedBelowAfterDuration
      );

      this.log.info(`On state is ${isOn ? 'ON' : 'OFF'}`);
